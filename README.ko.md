...
사업의 문제와 해법은 분명한데, 그것을 PDM 등 양식의 문장과 지표로 옮기려면 막막할 때가 있습니다.
변화이론 에이전트는 그 과정을 질문과 답변으로 함께 정리하는 도구입니다.

한 번에 한 가지씩 답하면 사회문제, 활동, 산출물, 성과, 영향으로 이어지는 결과사슬을 만듭니다.
국제개발협력 사업에는 **PDM(성과체계표)** 을, 임팩트 스타트업·CSR·비영리에는
**변화이론 도식**을 만듭니다. Claude 같은 AI 환경에서 사용할 수 있고, 대화 언어에 맞춰 답합니다.

단순히 칸을 채우지는 않습니다. 산출물을 성과로 쓴 경우처럼 결과사슬이 어긋난 부분을 짚어 줍니다.
핵심 검사는 코드로 실행됩니다. 품질 게이트는 벤치마크에서 **18/18**을 검출했고, 규칙에는
규칙 근거를 문서에서 확인할 수 있습니다. 예산 계산도 스크립트로 처리하며 예산서 두 건의 합계를 **원 단위까지** 재현했습니다.
...
1. 아래 방법으로 스킬을 설치합니다.
2. 채팅에 *"변화이론 에이전트로 PDM 만들어줘"*라고 입력합니다. 사업계획서나 연차보고서를 첨부해도 됩니다.
3. 질문에 답하고, 결과가 맞으면 **"확정"**이라고 말합니다.
4. `out/`에서 결과를 확인합니다. 국제개발협력은 `pdm.md`, 그 밖의 용도는 `toc.md`가 기본 결과물입니다.
...
새 PDM은 확정 전에 구조 검사를 거칩니다. 이미 승인된 PDM은 감사 모드로 확인할 수 있으며,
문서를 고치지 않고 가이드라인과 다른 부분만 보고합니다.
...
**인터뷰 → 결과사슬 구성 → 검토 → 검증 → 결과물 생성** 순서로 진행합니다.
...
| `details/pdm.json` | 각 결과물을 생성하는 원본 데이터 |

대화, PDF, 한글 파일(`.hwp`, `.hwpx`)로 시작할 수 있습니다. 한글 파일 추출기는 외부 의존성이 없어
앱의 코드 실행 환경에서도 동작합니다.
...
- **예산 계산:** 합계, 비율, 재원 분담, 일반관리비 상한은 스크립트가 계산하고 검증합니다. 예산서 두 건으로 세 차례 확인해 합계를 원 단위까지 재현했고 규칙 오류 세 건을 수정했습니다.
- **권장 규칙:** SMART, CREAM, 성별 분리 지표는 점수로만 알려 드립니다. 적용 여부는 사용자가 결정합니다.

## 설치: 30초

**준비물:** Claude Code, Claude 데스크톱 또는 claude.ai와 `python3`입니다. 데스크톱과 웹의 코드 실행 환경에는 Python이 포함되어 있습니다.
