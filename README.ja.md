...
単に様式を埋めるだけではありません。アウトプットをアウトカムとして書いているような論理のずれを
指摘します。検証も追跡できます。品質ゲートはベンチマークで **18/18** を検出し、各ルールには
ルールの根拠も文書で確認できます。予算計算はスクリプトで行い、予算書 2 件の合計を **ウォン単位まで**再現しました。
...
- **予算計算:** 合計、比率、資金分担、一般管理費上限はスクリプトが計算・検証します。予算書 2 件を 3 回検証し、ウォン単位まで再現しました。あわせてルールの不具合 3 件を修正しました。
